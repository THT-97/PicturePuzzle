...
Depth 7 steps: solved until 435760281 (solved 14)
*/
...
int* tryStep(int *puzzle, int dir){
	int i=0;
	while(puzzle[i]!=0) i++; //find piece 0
...
//debugged for traces lost
int estimateCost(int *puzzle, int currentStep, int lastStep, int estimated, int maxSteps){
	int c[] = {0,0,0,0};
	for(int i=0; i<4; i++){
		//if step is valid and is not in traces (avoid moving backward from last move)
		if(currentStep + lastStep != 0 && tryStep(puzzle, moves[i])!=NULL){
			if(!hadTrace(getKey(puzzle))){
				//add trace
				currentTrace += 1;
				trace[currentTrace] = getKey(puzzle);
				if(getCost(puzzle, currentStep)==currentStep){
					trace[currentTrace] = 0;
					currentTrace -= 1;
					tryStep(puzzle, -moves[i]); //undo step
					return 0; //if puzzle is solved
				}
				//deeper steps
				if(estimated<maxSteps) c[i] = getCost(puzzle, currentStep) +
					estimateCost(puzzle, currentStep+1, currentStep, estimated+1, maxSteps);
				//remove trace
				trace[currentTrace] = 0;
				currentTrace -= 1;
			}
...
void solvePuzzle(int* puzzle){
	...
	while(getKey(puzzle)!=picKey && currentTrace<MAX &&
		...
		for(int i=0; i<4; i++){
			//try possible steps to find min cost step
			if(tryStep(puzzle, moves[i])!= NULL){
				if(!hadTrace(getKey(puzzle))){
					...
					costs[i] = getCost(puzzle,step)+estimateCost(puzzle,step+1, step,1,6);
					//remove trace
					trace[currentTrace] = 0;
					currentTrace -= 1;
				}
		...
		int i = 0;
		while(i<4 && min != costs[i]) i++; //find min cost step
